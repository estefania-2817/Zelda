...
shared_ptr<Section> MapBuilder::createSection(int startingPixelMapX, int startingPixelMapY, shared_ptr<Link> &linkPtr, ofImage pixelMap) {
    ...
    for (int y = startingPixelMapY; y < startingPixelMapY + VERTICAL_COUNT; y++) {
        ...
        for (int x = startingPixelMapX; x < startingPixelMapX + HORIZONTAL_COUNT; x++) {
            ...
            // Walkable  Bonus Dungeon Floor
            else if (currentPixel == tilePixelColors.WALKABLE_BONUS_DUNGEON_FLOOR) {
                column = 4;
                row = 1;
                collidable = false;
            }
            // Recursive tiles
            else if (currentPixel == tilePixelColors.RECURSIVE_UP_FLOOR) {
                column =19;
                row = 0;
                collidable = false;
            }
            else if (currentPixel == tilePixelColors.RECURSIVE_DOWN_FLOOR) {
                column =20;
                row = 0;
                collidable = false;
            }
            else if (currentPixel == tilePixelColors.RECURSIVE_LEFT_FLOOR) {
                column =21;
                row = 0;
                collidable = false;
            }
            else if (currentPixel == tilePixelColors.RECURSIVE_RIGHT_FLOOR) {
                column =22;
                row = 0;
                collidable = false;
            }
            // Bonues Dungeon teleportation tiles
            else if (currentPixel == tilePixelColors.BONUS_DUNGEON_ONE) {
                column = 4;
                row = 10;
                collidable = true;
            }
            else if (currentPixel == tilePixelColors.BONUS_DUNGEON_TWO) {
                column =12;
                row = 10;
                collidable = true;
            }
            else if (currentPixel == tilePixelColors.BONUS_DUNGEON_TREE) {
                column =20;
                row = 10;
                collidable = true;
            }
            else if (currentPixel == tilePixelColors.BONUS_DUNGEON_FOUR) {
                column =28;
                row = 10;
                collidable = true;
            }
            ...
            // Bonus Dungeon Wall
            else if (currentPixel == tilePixelColors.BONUS_DUNGEON_WALL) {    /////////
                column = 18;    
                row = 0;      
            }
            ...
            // Cactus
            else if (currentPixel == tilePixelColors.CACTUS) {
                sprite0 = 2;
                collidable = true;
            }
            // OldMan's Cave Entrance
             ...
             // Dunegon's Entrance
                ...
                row = 1;
                isADoor = true;
                whereTo = MapName::NEW_DUNGEON;
            }
            // Bonus Dungeon's Enrance
            else if (currentPixel == entityPixelColors.BONUS_DUNGEON_ENTRANCE) {   //////////
                column = 4;
                row = 1;
                isADoor = true;
                whereTo = MapName::BONUS_DUNGEON;
            }
            ...
            else if (currentPixel == entityPixelColors.OVERWORLD_ENTRANCE2) {
                column = 4;
                row = 1;
                isADoor = true;
                whereTo = MapName::OVER_WORLD;
            }
            else if (currentPixel == entityPixelColors.OVERWORLD_ENTRANCE3) {  ////////
                column = 4;
                row = 1;
                isADoor = true;
                whereTo = MapName::OVER_WORLD;
            ...
            ofImage blockImage;
            if(sprite0 == 0) {
                blockImage =  handler->getImageManager()->getSpriteFromEntitySheetAt(column, row);
            } else if(sprite0 == 1) {
                blockImage =  handler->getImageManager()->getSpriteFromMapSheetAt(column, row);
            } else {
                blockImage = handler->getImageManager()->cactus;
            }
            ...
            if (collidable) // We found a block that is collidable; add it to the section
            {
                ...
                if (isADoor) // Oh shoot, it's a door instead
                {
                    ...
                    block = make_shared<BonusBlockOne>(windowXPos, windowYPos, handler);
                    
                }
                else if (currentPixel == tilePixelColors.BONUS_DUNGEON_TWO){
                    block = make_shared<BonusBlockTwo>(windowXPos, windowYPos, handler);
                }
                else if (currentPixel == tilePixelColors.BONUS_DUNGEON_TREE){
                    block = make_shared<BonusBlockTree>(windowXPos, windowYPos, handler);
                    
                }
                else if (currentPixel == tilePixelColors.BONUS_DUNGEON_FOUR){
                    block = make_shared<BonusBlockFour>(windowXPos, windowYPos, handler);
                }
                ...
                else if (currentPixel == entityPixelColors.FIRE)// FIRE
                {
                    ...
                    block = make_shared<Cactus>(windowXPos, windowYPos, handler); //Bono
                }
